...
//Used to inject the object into the anonymous callback function.
$apiFactory = new APIFactory();

/**
 * GET
 */
$router->get('/dataprocessing/api/v1/anime/{animeId}', function ($animeId) use ($apiFactory) { 
    return $apiFactory->executeSafeIdempotentQuery("getAnime", $animeId);
});
$router->get('/dataprocessing/api/v1/animes/', function () use ($apiFactory) {
    return $apiFactory->executeSafeIdempotentQuery("getAllAnimes");
});
$router->get('/dataprocessing/api/v1/ratings/animes/{animeid}/above/{higherthen}', function ($animeRatings, $higherThen) use ($apiFactory) {
    return $apiFactory->executeSafeIdempotentQuery("getAnimeRatings", $animeRatings, $higherThen);
});
$router->get('/dataprocessing/api/v1/ratings/above/{higherthen}', function ($higherThen) use ($apiFactory) {
    return $apiFactory->executeSafeIdempotentQuery("getAllAnimeRatings", $higherThen);
});
$router->get('/dataprocessing/api/v1/animes/top/{amount}', function ($amount) use ($apiFactory) {
    return $apiFactory->executeSafeIdempotentQuery("getTopWatchedAnime", $amount);
});
$router->get('/dataprocessing/api/v1/animes/top/', function () use ($apiFactory) {
    return $apiFactory->executeSafeIdempotentQuery("getAllTopWatchedAnimes");
});
$router->get('/dataprocessing/api/v1/anime/{animeid}/gendercomparison/', function ($animeId) use ($apiFactory) {
...
$router->get('/dataprocessing/api/v1/users/stats/anime/{animeId}', function ($animeId) use ($apiFactory) {
...
$router->put('/dataprocessing/api/v1/anime/{id}', function ($request, $id) use ($apiFactory) {
    return $apiFactory->executeNonSafeIdempotentQuery("putAnime", $id, $request->getBody());
});
